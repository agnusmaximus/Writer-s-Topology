...
CodeFlower.prototype.update = function(json) {
  ...
  this.node.transition()
      .attr("r", function(d) { return Math.pow((d.size + 3) * (d.size + 3) * Math.log((d.size + 30)), 2/5); });  
  ...
  this.node.enter().append('svg:circle')
	...
	return "rgba(" + 
	    ...
	    parseInt(Math.random() * (255 - 1) + 1) + " , " +
	    parseInt(Math.random() * (255 - 1) + 1) + " , " + 
	    parseFloat(d.size * d.size * 10 / 255) +")";
    })
...
CodeFlower.prototype.mouseover = function(d) {
    this.text.attr('transform', 'translate(' + d.x + ',' +  (d.y - 25 - (d.children ? 3.5 : Math.sqrt(d.size) / 2)) + ')')
	.text(d.name + " : " + d.size)
	.style('display', null);
};
